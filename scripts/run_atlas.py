...
N_HELDOUT = 15     # shuffled-target held-out null count
N_PERT    = 8      # random-perturbation null count
...
def nulls(name, d_full, cond, seed=0):
    ...
    return dict(heldout_cosine=ho.held_out_cosine, heldout_null_mean=ho.null_mean,
        ...
        pert_null_z=float((s.lof_cosine - pert.mean())/(pert.std()+1e-9)),
...
def main():
    ...
    for i,(name,cond) in enumerate(order,1):
        ...
        nl=nulls(name,TARGETS[name],cond); pt.update(nl); json.dump(pt,open(fp,"w"),indent=2)
