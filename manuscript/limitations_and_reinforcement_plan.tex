...
contribution: when a target is outside the measured knockdown cone, convex duality returns a
separating residual direction. Large positive coordinates identify unmet readouts and motivate
activation experiments, but they do not prove that activating those genes will generate the
target state. The full vector is the mathematical certificate; its coordinate ranking is a
biological hypothesis layer that has no direct wet-lab test in this work.
...
\begin{enumerate}
\item \emph{Direct test (wet-lab, weeks--months).} Run a small CRISPRa arm against selected
unmet-readout hypotheses and measure whether activation moves cells toward \Th{1}. A positive
result would support the coordinate-ranking heuristic; a negative result would not invalidate
the separating certificate, but would calibrate what its biological interpretation can support.
