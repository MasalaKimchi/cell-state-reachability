...
T-cell engineering, and it is the question of pharmaceutical target selection. Wrong target
or modality choice is one contributor to the cost of drug development: only about one in ten
clinical programmes reaches approval, and failure is often paid for late
...
returns a ranked mixture and greedy sparse panel, and ``outside the cone'' is a model-relative
theorem whose separating hyperplane---a Farkas/KKT certificate \citep{Farkas1902}---ranks
positive unmet readouts for follow-up rather than proving a per-gene activation recipe. We
validate by held-out-gene generalisation against a shuffled-target null
