...
hyperplane---a Farkas/KKT certificate \citep{Farkas1902}---whose positive coordinates name
...
feasibility \emph{relative to the measured dictionary} under an explicit additivity model,
and its verdicts are hypotheses to be tested at the bench, not validated targets. We are
correspondingly conservative about what we claim. We report held-out-gene cosines against a
shuffled-target null rather than in-sample fits, because the in-sample number is optimistic;
we show that the absolute reachable cosine of a real polarization shift is modest
($0.448$ for T\textsubscript{H}2$\rightarrow$T\textsubscript{H}1), which is the honest
ceiling of loss-of-function acting alone, not a defect of the method; and we verify on
measured double perturbations that the additivity assumption underpinning multi-gene recipes
is a bounded approximation rather than an identity. The regulators the oracle surfaces in
T cells and in the CEBPA program are already reported by the source screens
\citep{Zhu2025,Norman2019}; we use them only as positive controls, and claim novelty for the
measured-effect, certificate-carrying reachability \emph{decision} itself.
...
\item \textbf{A validated headline result with calibrated honesty.} On primary human
