...
additive) should not certify. The synthetic screen gives the in-silico version ($0/80$ false
positives); the dossier adds the real-data version---across $150$ additive combinations synthesized
from measured Norman single-gene effects, $0$ are false-certified---and reports a per-pair
\emph{sensitivity radius} $\Gamma^\star$ (median $\approx 1.25\times$ noise inflation to overturn a
certified verdict), the certificate's analogue of a Rosenbaum bound. The dossier also separates the
two quantities the certificate returns: the cone residual is the effect-\emph{size} axis (it recovers
classical synergy at partial Spearman $0.62$ controlling for magnitude), whereas the $z$-score is a
noise-relative \emph{confidence} that is decorrelated from magnitude by design---so a verdict must be
read with its residual, never as a size. Fourth, \emph{guide non-compliance}: incomplete knockdown
